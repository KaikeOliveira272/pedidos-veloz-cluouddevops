###Como rodar na sua máquina (Local) Para subir a aplicação inteira com um comando só, usamos o Docker Compose:
...
###Rodando no Kubernetes (K8s) O ambiente de produção foi desenhado para rodar em cluster. Os arquivos de configuração estão na pasta /k8s.
...
###Monitoramento e Tracing Para a parte de observabilidade avançada, a proposta é usar:
...
###Estratégia de Deploy Escolhemos o Rolling Update. Assim, o site não fica fora do ar enquando a gente sobe uma versão nova, reduzindo o risco de erro que a empresa tinha antes.
